...
#gam clasificacion se dice con familia binomial, variable de salida en rango (0,1)
...
a <- round(predict(model0, newdata = datos, type="response")) #predict devuelve entre 0 y 1
Acierto(datos[,1],a)
...
df
#modelo3 y 4 problema de sobreajuste, mirar CV
...
datos <-data.frame(y=as.numeric(iris$Species),
                   x1=iris$Sepal.Length,
                   x2=iris$Petal.Length,
                   x3=iris$Petal.Width,
                   x4=iris$Sepal.Width)
...
data1<-datos
data1$y<-ifelse(datos$y==1,1,0)
data2<-datos
data2$y<-ifelse(datos$y==2,1,0)
data3<-datos
data3$y<-ifelse(datos$y==3,1,0)
modelGAM1 <-gam(y~ns(x1,4)+ns(x2,4)+ns(x3,4)+ns(x4,4), family="binomial", data= data1) 
modelGAM2 <-gam(y~ns(x1,4)+ns(x2,4)+ns(x3,4)+ns(x4,4), family="binomial", data= data2) 
modelGAM3 <-gam(y~ns(x1,4)+ns(x2,4)+ns(x3,4)+ns(x4,4), family="binomial", data= data3) 


#analisis modelGAM1
a1 <- round(predict(modelGAM1, newdata = data1, type="response")) #predict devuelve entre 0 y 1
Acierto(data1[,1],a1)
ValidacionCruzada(data1,10,modelGAM1)

#analisis modelGAM2
a2 <- round(predict(modelGAM2, newdata = data2, type="response")) #predict devuelve entre 0 y 1
Acierto(data2[,1],a2)
ValidacionCruzada(data2,10,modelGAM2)

#analisis modelGAM3
a3 <- round(predict(modelGAM3, newdata = data3, type="response")) #predict devuelve entre 0 y 1
Acierto(data3[,1],a3)
ValidacionCruzada(data3,10,modelGAM3)

#juntamos los modelos
datosFinal<-datos
datosFinal$y=0
datosFinal$y<-a1
datosFinal$y<-ifelse(datosFinal$y==0,a3*3,a1)
datosFinal$y<-ifelse(datosFinal$y==0,2,datosFinal$y)

#comparacion
plot(ifelse(datosFinal$y==datos$y,datosFinal$y,NA),col=c("green"))
points(ifelse(datosFinal$y!=datos$y,datosFinal$y,NA),col=c("red"))
#% acierto
sum(datosFinal$y==datos$y)/length(datosFinal$y)*100
