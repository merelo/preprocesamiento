...
bd <- read.csv("practica3/CoordenadasMunicipios.csv", header = T, sep = ",")
...
SmT =predict (mT ,train ,type ="class")
SmTtest =predict (mT ,test ,type ="class")
...
z.pred<-seq(min(datos$x1),max(datos$x1),length.out = grid.lines)
xz<-expand.grid(x2=x.pred,x1=z.pred)
salidaMT2<-predict(mT,newdata=xz,type="class")
plot(xz[,2],xz[,1],col=salidaMT2,pch="*")
points(train$x1,train$x2,col=SmT,pch="x")
...
rF <- randomForest(y ~ x1 + x2, data=train)
SrF =predict (rF ,train ,type ="class")
SrFtest =predict (rF ,test ,type ="class")

Acierto(train$y,SrF)
Acierto(test$y,SrFtest)
plot(train$x1,train$x2,col=train$y,pch=0)
points(train$x1,train$x2,col=SrF,pch="x")
x.pred<-seq(min(datos$x2),max(datos$x2),length.out = grid.lines)
z.pred<-seq(min(datos$x1),max(datos$x1),length.out = grid.lines)
xz<-expand.grid(x2=x.pred,x1=z.pred)
salidaRF3<-predict(rF,newdata=xz,type="class")
plot(xz[,2],xz[,1],col=salidaRF3,pch="*")
points(train$x1,train$x2,col=SrF,pch="x")
...
library (gbm)
narb=5000
boost =gbm(y ~ x1 + x2,data=train, 
                distribution="multinomial",n.trees =narb,
                interaction.depth =4)
Sb =predict (boost ,train,n.trees =narb)
Sbtest =predict (boost ,test,n.trees =narb)

Acierto(train$y,Sb)
Acierto(test$y,Sbtest)
plot(train$x1,train$x2,col=train$y,pch=0)
points(train$x1,train$x2,col=Sb,pch="x")
x.pred<-seq(min(datos$x2),max(datos$x2),length.out = grid.lines)
z.pred<-seq(min(datos$x1),max(datos$x1),length.out = grid.lines)
xz<-expand.grid(x2=x.pred,x1=z.pred)
salidaB4<-predict(boost,newdata=xz,n.trees=narb)
plot(xz[,2],xz[,1],col=salidaB4,pch="*")
points(train$x1,train$x2,col=Sb,pch="x")
